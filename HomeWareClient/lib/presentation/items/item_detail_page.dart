...
class _ItemDetailPageState extends ConsumerState<ItemDetailPage> {
  ...
  void _refresh() {
    ...
    ref.invalidate(itemSales7dProvider(widget.id));
          ...
          actions: [
            PopupMenuButton<String>(
              ...
              itemBuilder: (context) {
                ...
                final canDelete = ShopRoleGuard.canDeleteItem(skin, role);
                return [
  ...
  Widget _buildDetailList(BuildContext context, ItemDetailData data) {
    ...
    final content = Column(
      children: [
        ...
        _detailRow('💰', skin.purchasePriceLabel, priceLine),
        if (skin.showSalePrice)
          _detailRow(
            '🏷️',
            skin.salePriceLabel,
            skin.formatSalePrice(salePrice: item.salePrice, unit: item.unit),
          ),
        if (skin.showSupplier && (item.supplier?.isNotEmpty ?? false))
          _detailRow('🚚', skin.supplierLabel, item.supplier ?? '—'),
        if (skin.showSalePrice)
          _detailRow(
            '📊',
            '近7日卖出',
            ref.watch(itemSales7dProvider(widget.id)).when(
                  data: (sales) => skin.formatItemSales7d(
                    sellTimes: sales.sellTimes,
                    sellQuantity: sales.sellQuantity,
                    unit: item.unit,
                    revenue: sales.revenue,
                    revenueComplete: sales.revenueIsComplete,
                    grossProfit: sales.grossProfit,
                    costIsComplete: sales.costIsComplete,
                  ),
                  loading: () => '加载中…',
                  error: (_, __) => '—',
                ),
  ...
  Future<void> _onRepurchase(Item item) async {
    ...
    debugPrint('[ItemDetailPage] INFO: 加入清单 id=${item.id}');
    if (mounted) {
      final skin = ref.read(spaceSkinProvider);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('「${item.name}」已加入${skin.shoppingListLabel}')),
      );
    }
  }
  ...
  String _usageDesc(UsageRecord record) {
    ...
    switch (record.type) {
      ...
      case 1:
        return ref.read(spaceSkinProvider).usageConsumeLine(
